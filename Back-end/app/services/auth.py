...
class AuthService:
    ...
    def login(db: Session, credentials: UserLogin) -> dict:
        ...
        access_token = create_access_token(data={"sub": str(user.id), "email": user.email, "role": role_name or "user"})
    ...
    @staticmethod
    def admin_create_user(db: Session, email: str, password: str, full_name: str, role: str = 'user') -> User:
