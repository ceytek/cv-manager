...
async def get_company_id(
    ...
    if not company_id_str:
        return None  # No company context (might be admin or legacy user)
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Geçersiz şirket ID"
        )


async def require_company_id(
    company_id: Optional[UUID] = Depends(get_company_id)
) -> UUID:
    """Require company_id in token (fail if not present)"""
    
    if not company_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Bu işlem için şirket bağlamı gerekli"
        )
...
def get_company_id_from_token(token: str) -> Optional[UUID]:
    """Extract company_id from raw token (synchronous helper for GraphQL)"""
