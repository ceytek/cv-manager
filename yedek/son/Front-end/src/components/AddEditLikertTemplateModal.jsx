...
const AddEditLikertTemplateModal = ({ isOpen, onClose, onSuccess, template }) => {
  ...
  useEffect(() => {
    if (templateData?.likertTemplate) {
      ...
      console.log('[LikertTemplate] Loading template data:', { timeLimit: t.timeLimit, hasTimeLimit: !!t.timeLimit });
      setName(t.name || '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('[LikertTemplate] Saving with input:', { hasTimeLimit, timeLimit, inputTimeLimit: input.timeLimit });

      if (isEdit) {
        const result = await updateTemplate({ variables: { id: template.id, input } });
        console.log('[LikertTemplate] Update result:', result);
        console.log('[LikertTemplate] Returned timeLimit:', result?.data?.updateLikertTemplate?.template?.timeLimit);
      } else {
        const result = await createTemplate({ variables: { input } });
        console.log('[LikertTemplate] Create result:', result);
        console.log('[LikertTemplate] Returned timeLimit:', result?.data?.createLikertTemplate?.template?.timeLimit);
      }
      onSuccess?.();
    } catch (err) {
